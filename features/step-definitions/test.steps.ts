...
When(
  `User enters {string} and checks {string} checkbox`,
  async (username, arg0: string) => {
...
When(
  `User enters Nombre {string} and Telefono de contacto  {string}`,
  async (username, telNo) => {
    await pages.test.enterUsername(username);
...
When(`User selects time as {string}`, async (time) => {
  await pages.test.setAppointmentTime(time);
  await browser.pause(2000);
});
